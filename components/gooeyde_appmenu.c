...
static void calculate_dynamic_layout(void)
{
    ...
    if (layout.app_card_width < 200)
        layout.app_card_width = 200;
    if (layout.app_card_width > 350)
        layout.app_card_width = 350;
    if (layout.app_card_height < 80)
        layout.app_card_height = 80;
    if (layout.app_card_height > 140)
        layout.app_card_height = 140;
    ...
    if (layout.title_font_size < 20)
        layout.title_font_size = 20;
    if (layout.title_font_size > 32)
        layout.title_font_size = 32;
    layout.label_font_size = base_height / 54;
    if (layout.label_font_size < 10)
        layout.label_font_size = 10;
    if (layout.label_font_size > 14)
        layout.label_font_size = 14;
    layout.nav_button_width = 120;
    layout.nav_button_height = 40;
    layout.icon_size = layout.app_card_height - 30;
    if (layout.icon_size < 32)
        layout.icon_size = 32;
    if (layout.icon_size > 64)
        layout.icon_size = 64;
    layout.search_width = ((float)base_width / 2.5);
    layout.search_height = 50;
...
static void perform_fuzzy_search(const char *query)
{
    ...
    for (size_t i = 0; i < app_count; i++)
    {
            ...
            (detected_apps[i].categories && fuzzy_match(detected_apps[i].categories, query)))
        {
            ...
            filtered_apps[idx].icon_path = detected_apps[i].icon_path ? strdup(detected_apps[i].icon_path) : NULL;
            filtered_apps[idx].categories = detected_apps[i].categories ? strdup(detected_apps[i].categories) : NULL;
            idx++;
        }
    }
...
static void create_ui(void)
{
    ...
    for (int row = 0; row < layout.grid_rows; row++)
    {
        for (int col = 0; col < layout.grid_cols; col++)
        {
            ...
            app_icons[grid_index] = GooeyImage_Create("/usr/local/share/gooeyde/assets/unknown.png",
                                                      x + 10, y + 25,
...
static void update_ui(void)
{
    ...
    for (int i = 0; i < layout.apps_per_page; i++)
    {
        ...
        if (app_buttons[i])
            GooeyWidget_MakeVisible(app_buttons[i], is_visible);
        if (app_icons[i])
        {
            if (is_visible)
            {
                if (current_apps[app_index].icon_path)
                {
                ...
                else
                {
                    ...
                    GooeyImage_SetImage(app_icons[i], "/usr/local/share/gooeyde/assets/unknown.png");
                    GooeyWidget_MakeVisible(app_icons[i], 1);
                }
            }
            else
...
int main(int argc, char **argv)
{
    ...
    main_window->active_theme->widget_base = 0x4a4a8a;
    main_window->vk = NULL;
    create_ui();
